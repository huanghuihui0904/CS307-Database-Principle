<template>
  <div style="width: 100vw;padding-left: 10px;padding-right: 10px"  >
    ...
    <div style="margin-outside: 10px">
      <!--      <el-input v-model="search" placeholder="请输入搜索的列" style="width: 25%;padding: 10px;"></el-input>-->
      <el-input v-model="content" placeholder="请输入搜索的关键词" style="width: 25%; padding: 10px" ></el-input>
      <el-button type="primary" style="margin-left: 0"
                 @click="$router.push('./center')">搜索</el-button>
      <el-button type="primary" @click="getCount">总计</el-button>
...
export default {
  ...
  methods: {
    ...
    update(row){
      const _this=this
      axios.get('/center1/findById/'+row.id).then(function (resp){
    ...
    create() {
      const _this=this
      axios.get('/center1/findById/'+this.$route.query.id).then(function (resp){
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios.post("/center1/save", this.ruleForm).then(function (resp) {
    ...
    updateForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios.put("/center1/update", this.ruleForm2).then(function (resp) {
    ...
    getCount(){
      const _this = this
      axios.get('/center1/get' ).then(function (resp) {
        console.log(resp)
    ...
    deleteCenter(row){
      const _this = this
      axios.delete('/center1/deleteById/' + row.id).then(function (resp) {
        _this.$alert('删除成功!', 'OK', {
          confirmButtonText: '确定',
          callback: action => {
    ...
    page(currentPage) {
      const _this = this
      axios.get('/center1/findAll/' + currentPage + '/10').then(function (resp) {
